...
unsigned long softwalk_page_table(unsigned long addr)
{
    ...
    //printk("Page-Address: %016lx VA: %016lx Mask: %016lx 12bits: %016lx \n", pa, addr, mask, last12bits);
    pa = pa | last12bits;
...
/* 
    ...
    Handler Function
*/
void AccessAndMap(void)
{
    unsigned long pa1, pa2;

    p = (struct node *) va; 
    printk("Node-P: %p VA: %lu \n", p, va);    
    printk("Value at Node: %d \n", p->data);
...
{
    printk("Part-2-Access: -----------------------INSERTED-------------------------\n");
    
    if(va)
        AccessAndMap();
    else
        printk("Pass VA as module parameter");
